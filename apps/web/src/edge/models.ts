...
export async function loadModelMesh(key: string, env: Env, ctx: ExecutionContextLike): Promise<ModelLookup> {
  ...
  try {
    ...
    return { kind: "error", status: 422, message: `could not convert model: ${(error as Error).message}` };
  }
  if (env.MODELS) {
    ctx.waitUntil(
...
export async function handleModel(request: Request, env: Env, ctx: ExecutionContextLike, url: URL): Promise<Response> {
  ...
  if (found.kind === "error") return new Response(found.message, { status: found.status, headers: CORS });
