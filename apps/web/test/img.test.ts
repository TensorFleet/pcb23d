...
describe("fillPage", () => {
  test("points every slot and og tag at the repo's render", async () => {
    ...
    expect(html).toContain('<meta property="og:image" content="https://pcbto3d.com/img/gh/bob/nancy/@main.jpg" />');
    expect(html).toContain('<meta property="og:image:width" content="1200" />');
    expect(html).toContain('<meta property="og:image:height" content="630" />');
    expect(html).toContain('<meta name="twitter:image" content="https://pcbto3d.com/img/gh/bob/nancy/@main.jpg" />');
    expect(html).toContain('<meta property="og:url" content="https://pcbto3d.com/img/gh/bob/nancy/@main" />');
    expect(html).toContain('<link rel="canonical" href="https://pcbto3d.com/img/gh/bob/nancy/@main" />');
    expect(html).toContain('<h1 data-slot="repo">bob/nancy@main</h1>');
    expect(html).toContain('<img data-slot="image" alt="3D render of bob/nancy@main" src="https://pcbto3d.com/img/gh/bob/nancy/@main.jpg" />');
    expect(html).toContain('<a data-view="top" href="https://pcbto3d.com/img/gh/bob/nancy/@main.jpg?view=top">');
...
describe("renderBoardImage", () => {
  const fixture = Bun.file(new URL("../../../packages/core/test/fixtures/constraints.kicad_pcb", import.meta.url));
  const ctx = { waitUntil() {} };
  test("encodes a JPEG social card without touching the network for a model-free board", async () => {
    ...
    const jpg = await renderBoardImage(board, "constraints.kicad_pcb", { ...OG_RENDER, width: 120, height: 63 }, { ASSETS: { fetch: async () => new Response() } }, ctx);
    expect([...jpg.subarray(0, 3)]).toEqual([0xff, 0xd8, 0xff]);
    const png = await renderBoardImage(board, "constraints.kicad_pcb", { ...OG_RENDER, width: 120, height: 63, format: "png" }, { ASSETS: { fetch: async () => new Response() } }, ctx);
